...
(function (global, factory) {
	...
	(function() {
		var current = global.blcktive;
		var exports = factory();
		global.blcktive = exports;
		exports.noConflict = function() { global.blcktive = current; return exports; };
	})();
...
if (hasConsole) {
  var welcomeIntro = [
    "%cblcktive.js %c1.4.1 %cin debug mode, %cmore...",
    'color: rgb(114, 157, 52); font-weight: normal;',
    'color: rgb(85, 85, 85); font-weight: normal;',
    'color: rgb(85, 85, 85); font-weight: normal;',
    'color: rgb(82, 140, 224); font-weight: normal; text-decoration: underline;'
  ];
  var welcomeMessage = "You're running blcktive 1.4.1 in debug mode - messages will be printed to the console to help you fix problems and optimise your application.\n\nTo disable debug mode, add this line at the start of your app:\n  blcktive.DEBUG = false;\n\nTo disable debug mode when your app is minified, add this snippet:\n  blcktive.DEBUG = /unminified/.test(function(){/*unminified*/});\n\nGet help and support:\n  http://blcktive.js.org\n  http://stackoverflow.com/questions/tagged/ungtb10d\n  http://groups.google.com/forum/#!forum/blcktive-js\n  http://twitter.com/ungtb10d\n\nFound a bug? Raise an issue:\n  https://github.com/ungtb10d/blcktive/issues\n\n";

  welcome = function () {
    if (blcktive.WELCOME_MESSAGE === false) {
      welcome = noop;
      return;
    }
    var message = 'WELCOME_MESSAGE' in blcktive ? blcktive.WELCOME_MESSAGE : welcomeMessage;
  ...
  printWarning = function (message, args) {
    ...
    if (isObjectType(args[args.length - 1])) {
      ...
      var blcktive = options ? options.blcktive : null;

      if (blcktive) {
        // if this is an instance of a component that we know the name of, add
        // it to the message
        var name;
        if (blcktive.component && (name = blcktive.component.name)) {
        ...
        if (
          (node =
            options.node || (blcktive.fragment && blcktive.fragment.rendered && blcktive.find('*')))
        ) {
          args.push(node);
        }
      }
    ...
    console.warn.apply(
      console,
      ['%cblcktive.js: %c' + message, 'color: rgb(114, 157, 52);', 'color: rgb(85, 85, 85);'].concat(
        args
      )
    );
  };
...
var missingPlugin = function (name, type) { return ("Missing \"" + name + "\" " + type + " plugin. You may need to download a plugin via http://blcktive.js.org/integrations/#" + type + "s"); };

function findInViewHierarchy(registryName, blcktive, name) {
  var instance = findInstance(registryName, blcktive, name);
  return instance ? instance[registryName][name] : null;
}

function findInstance(registryName, blcktive, name) {
  while (blcktive) {
    if (name in blcktive[registryName]) {
      return blcktive;
    }

    if (blcktive.isolated) {
      return null;
    }

    blcktive = blcktive.parent;
  }
}

function interpolate(from, to, blcktive, type) {
  if (from === to) { return null; }

  if (type) {
    var interpol = findInViewHierarchy('interpolators', blcktive, type);
...
ModelBase__proto__.getKeypath = function getKeypath (blcktive) {
  if (blcktive !== this.blcktive && this._link) { return this._link.target.getKeypath(blcktive); }

  if (!this.keypath) {
    var parent = this.parent && this.parent.getKeypath(blcktive);
    this.keypath = parent
      ? ((this.parent.getKeypath(blcktive)) + "." + (escapeKey(this.key)))
      : escapeKey(this.key);
  }
...
ModelBase__proto__.has = function has (key) {
  ...
  computed = this.root.blcktive && this.root.blcktive.computed;
  if (computed) {
...
function maybeBind(model, value, shouldBind) {
  if (shouldBind && isFunction(value) && model.parent && model.parent.isRoot) {
    if (!model.boundValue) {
      model.boundValue = bind(value._r_unbound || value, model.parent.blcktive);
    }
...
var LinkModel = (function (ModelBase) {
  ...
  LinkModel__proto__.getKeypath = function getKeypath (blcktive) {
    if (blcktive && blcktive !== this.root.blcktive) { return this.target.getKeypath(blcktive); }

    return ModelBase.prototype.getKeypath.call(this, blcktive);
  };
...
function readReference(parser) {
  ...
  // bug out if it's a keyword (exception for ancestor/restricted refs - see https://github.com/ungtb10d/blcktive/issues/1497)
...
// See https://github.com/ungtb10d/template-spec for information
// about the blcktive template specification
...
var TEMPLATE_INSTRUCTIONS = "Either preparse or use a blcktive runtime source that includes the parser. ";

var COMPUTATION_INSTRUCTIONS = "Either include a version of blcktive that can parse or convert your computation strings to functions.";

function throwNoParse(method, error, instructions) {
  if (!method) {
    fatal(("Missing blcktive.parse - cannot parse " + error + ". " + instructions));
  }
}
...
var parser = {
  fromId: function fromId(id, options) {
    if (!doc) {
      ...
      throw new Error(("Cannot retrieve template #" + id + " as blcktive is not running in a browser."));
    }
  ...
  getParseOptions: function getParseOptions(blcktive) {
    // Could be blcktive or a Component
    if (blcktive.defaults) {
      blcktive = blcktive.defaults;
    }

    return parseOptions.reduce(function (val, key) {
      val[key] = blcktive[key];
      return val;
    }, {});
  },
  ...
  parseFor: function parseFor(template, blcktive) {
    return this.parse(template, this.getParseOptions(blcktive));
  }
};

function getComputationSignature(blcktive, key, signature) {
  var getter;
  var setter;
  ...
  if (isFunction(signature)) {
    getter = bind(signature, blcktive);
    getterString = signature.toString();
    getterUseStack = true;
  }

  if (isString(signature)) {
    getter = createFunctionFromString(signature, blcktive);
    getterString = signature;
  }

  if (isObjectType(signature)) {
    if (isString(signature.get)) {
      getter = createFunctionFromString(signature.get, blcktive);
      getterString = signature.get;
    } else if (isFunction(signature.get)) {
      getter = bind(signature.get, blcktive);
    ...
    if (isFunction(signature.set)) {
      setter = bind(signature.set, blcktive);
...
var Model = (function (ModelBase) {
  ...
  Model__proto__.adapt = function adapt () {
    ...
    var blcktive = this.root.blcktive;
    ...
    for (i = 0; i < len; i += 1) {
      ...
      if (adaptor.filter(value, keypath, blcktive)) {
        this$1.wrapper = adaptor.wrap(blcktive, value, keypath, getPrefixer(keypath));
  ...
  Model__proto__.compute = function compute (key, computed) {
    ...
    if (registry[key]) {
      registry[key].signature = getComputationSignature(this.root.blcktive, key, computed);
      registry[key].mark();
    } else {
      registry[key] = new shared$1.Computation(
        this,
        getComputationSignature(this.root.blcktive, key, computed),
        key
      );
    }
  ...
  Model__proto__.joinKey = function joinKey (key, opts) {
    ...
    if (!child) {
      var computed;
      if (this.isRoot && this.blcktive && (computed = this.blcktive.computed[key])) {
        child = this.compute(key, computed);
      } else if (!this.isRoot && this.root.blcktive) {
        var registry = this.root.blcktive.computed;
...
var SharedModel = (function (Model) {
  function SharedModel(value, name, blcktive) {
...
function resolveReference(fragment, ref) {
  ...
  // ancestor references
  if (ref[0] === '~') { return fragment.blcktive.viewmodel.joinAll(splitKeypath(ref.slice(2))); }
  ...
  if (base[0] === '@') {
    ...
    // @this referring to local blcktive instance
    if (base === '@this' || base === '@') {
      return fragment.blcktive.viewmodel.getblcktiveModel().joinAll(keys$$1);
      ...
      var root = ref[1] === 'r' ? fragment.blcktive.root : null;
      var f = fragment;

      while (
        f &&
        (!f.context || (f.isRoot && f.blcktive.component && (root || !f.blcktive.isolated)))
      ) {
      ...
      // @style shared model
      return fragment.blcktive.constructor._cssModel.joinAll(keys$$1);
    } else if (base === '@helpers') {
      // @helpers instance model
      return fragment.blcktive.viewmodel.getHelpers().joinAll(keys$$1);
    } else if (base === '@macro') {
  ...
  if (base && !keys$$1.length) {
    var helpers = fragment.blcktive.viewmodel.getHelpers();
  ...
  var shouldWarn = fragment.blcktive.warnAboutAmbiguity;
  var crossed = 0;
  var model;
  ...
  while (fragment) {
    ...
    if (model) {
      if (createMapping) {
        model = initialFragment.blcktive.viewmodel.createLink(base, model, base, { implicit: true });
      }
    ...
    if (
      !fragment.blcktive.isolated &&
      !(fragment.owner && fragment.owner.containerFragment) &&
      (fragment.componentParent || (!fragment.parent && fragment.blcktive.component))
    ) {
      // ascend through component boundary
      fragment = fragment.componentParent || fragment.blcktive.component.up;
      createMapping = true;
    } else {
  ...
  var instance = initialFragment.blcktive;
...
function getblcktiveContext(blcktive) {
  var assigns = [], len = arguments.length - 1;
  while ( len-- > 0 ) assigns[ len ] = arguments[ len + 1 ];

  var fragment =
    blcktive.fragment ||
    blcktive._fakeFragment ||
    (blcktive._fakeFragment = new FakeFragment(blcktive));
...
var FakeFragment = function FakeFragment(blcktive) {
  this.blcktive = blcktive;
};

FakeFragment.prototype.findContext = function findContext () {
  return this.blcktive.viewmodel;
};
...
function findParentWithContext(fragment) {
  ...
  if (!frag) { return fragment && fragment.blcktive.fragment; }
  else { return frag; }
}
...
var star = /\*/;
function gather(blcktive, keypath, base, isolated) {
  ...
  var model = base || blcktive.viewmodel;

  if (star.test(keypath)) {
    return model.findMatches(keys$$1);
  } else {
    if (model === blcktive.viewmodel) {
      ...
      if (
        ...
        return [resolveReference(blcktive.fragment || new FakeFragment(blcktive), keypath)];
      } else {
...
function build(blcktive, keypath, value, isolated) {
  var sets = [];
  ...
  if (isObject(keypath)) {
    var loop = function ( k ) {
      if (hasOwn(keypath, k)) {
        sets.push.apply(sets, gather(blcktive, k, null, isolated).map(function (m) { return [m, keypath[k], k]; }));
      }
    };
    ...
    sets.push.apply(sets, gather(blcktive, keypath, null, isolated).map(function (m) { return [m, value, keypath]; }));
  }
...
function add(blcktive, keypath, d, options) {
  if (!isString(keypath) || !isNumeric(d)) {
    throw new Error('Bad arguments');
  }

  var sets = build(blcktive, keypath, d, options && options.isolated);
...
function blcktive$add(keypath, d, options) {
...
function animate(blcktive, model, to, options) {
  options = getOptions(options, blcktive);
  var from = model.get();
  ...
  var interpolator = interpolate(from, to, blcktive, options.interpolator);
...
function blcktive$animate(keypath, to, options) {
  if (isObjectType(keypath)) {
    var keys$$1 = keys(keypath);

    throw new Error(("blcktive.animate(...) no longer supports objects. Instead of blcktive.animate({\n  " + (keys$$1.map(function (key) { return ("'" + key + "': " + (keypath[key])); }).join('\n  ')) + "\n}, {...}), do\n\n" + (keys$$1.map(function (key) { return ("blcktive.animate('" + key + "', " + (keypath[key]) + ", {...});"); }).join('\n')) + "\n"));
  }

  return animate(this, this.viewmodel.joinAll(splitKeypath(keypath)), to, options);
}

function enqueue(blcktive, event) {
  if (blcktive.event) {
    blcktive._eventQueue.push(blcktive.event);
  }

  blcktive.event = event;
}

function dequeue(blcktive) {
  if (blcktive._eventQueue.length) {
    blcktive.event = blcktive._eventQueue.pop();
  } else {
    blcktive.event = null;
  }
}
...
function fireEvent(blcktive, eventName, context, args) {
  if ( args === void 0 ) args = [];
  ...
  var eventNames = blcktive._nsSubs ? variants(eventName, true) : ['*', eventName];

  return fireEventAs(blcktive, eventNames, context, args, true);
}

function fireEventAs(blcktive, eventNames, context, args, initialFire) {
  if ( initialFire === void 0 ) initialFire = false;

  var bubble = true;

  if (initialFire || blcktive._nsSubs) {
    enqueue(blcktive, context);

    var i = eventNames.length;
    while (i--) {
      if (eventNames[i] in blcktive._subs) {
        bubble = notifySubscribers(blcktive, blcktive._subs[eventNames[i]], context, args) && bubble;
      }
    }
  ...
  if (blcktive.parent && bubble) {
    if (initialFire && blcktive.component) {
      var fullName = blcktive.component.name + '.' + eventNames[eventNames.length - 1];
      eventNames = variants(fullName, false);

      if (context && !context.component) {
        context.component = blcktive;
      }
    }

    bubble = fireEventAs(blcktive.parent, eventNames, context, args);
  }

  return bubble;
}

function notifySubscribers(blcktive, subscribers, context, args) {
  ...
  for (var i = 0, len = subscribers.length; i < len; i += 1) {
    if (!subscribers[i].off && subscribers[i].handler.apply(blcktive, args) === false) {
      stopEvent = true;
    }
  }
...
Hook.prototype.fire = function fire (blcktive, arg) {
  var context = getblcktiveContext(blcktive);
  var method = this.method;

  if (blcktive[method]) {
    arg ? blcktive[method](context, arg) : blcktive[method](context);
  }

  fireEvent(blcktive, this.event, context, arg ? [arg, blcktive] : [blcktive]);
};

function getChildQueue(queue, blcktive) {
  return queue[blcktive._guid] || (queue[blcktive._guid] = []);
}

function fire(hookQueue, blcktive) {
  var childQueue = getChildQueue(hookQueue.queue, blcktive);

  hookQueue.hook.fire(blcktive);
  ...
  delete hookQueue.queue[blcktive._guid];
}
...
HookQueue__proto__.begin = function begin (blcktive) {
  this.inProcess[blcktive._guid] = true;
};

HookQueue__proto__.end = function end (blcktive) {
  var parent = blcktive.parent;
  ...
  if (!parent || !this.inProcess[parent._guid]) {
    ...
    getChildQueue(this.queue, parent).push(blcktive);
  }

  delete this.inProcess[blcktive._guid];
};
...
function findAnchors(fragment, name) {
  ...
  findAnchorsIn(fragment, name, res, fragment.blcktive);
...
function findAnchorsIn(item, name, result, instance) {
  if (item.isAnchor) {
  ...
  } else if (item.fragment && (!item.component || item.fragment.blcktive === instance)) {
    ...
    anchors.forEach(function (a) { return a.blcktive === instance && result.push(a); });
  }
}
...
function attachChild(child, options) {
  ...
  var meta = {
    instance: child,
    blcktive: this,
    name: options.name || child.constructor.name || 'blcktive',
  ...
  var rm = child.viewmodel.getblcktiveModel();
  rm.joinKey('parent', { lastLink: false }).link(this.viewmodel.getblcktiveModel());
  rm.joinKey('root', { lastLink: false }).link(this.root.viewmodel.getblcktiveModel());
...
function blcktive$compute(path, computed) {
  var promise = runloop.start();
...
function blcktive$detach() {
  if (this.isDetached) {
    return this.el;
  }

  if (this.el) {
    removeFromArray(this.el.__blcktive_instances__, this);
  }
...
function blcktive$find(selector, options) {
  var this$1 = this;
  if ( options === void 0 ) options = {};

  if (!this.rendered)
    { throw new Error(
      ("Cannot call blcktive.find('" + selector + "') unless instance is rendered to the DOM")
    ); }
...
function blcktive$findAll(selector, options) {
  if ( options === void 0 ) options = {};

  if (!this.rendered)
    { throw new Error(
      ("Cannot call blcktive.findAll('" + selector + "', ...) unless instance is rendered to the DOM")
    ); }
...
function blcktive$findAllComponents(selector, options) {
...
function blcktive$findComponent(selector, options) {
  var this$1 = this;
  if ( options === void 0 ) options = {};
...
function blcktive$findContainer(selector) {
  if (this.container) {
...
function blcktive$findParent(selector) {
  if (this.parent) {
...
// sections when you do e.g. `blcktive.splice( 'items', 2, 2 );
...
function update$1(blcktive, model, options) {
  ...
  hooks.update.fire(blcktive, model);

  return promise;
}

function blcktive$update(keypath, options) {
  var opts, path;
...
var ContextData = (function (Model) {
  function ContextData(options) {
    ...
    this.root = this;
    this.value = {};
    this.blcktive = options.blcktive;
    this.adaptors = [];
...
var Context = function Context(fragment, element) {
  ...
  this.blcktive = fragment.blcktive;
  this.root = this;
};
...
Context__proto__.animate = function animate$1 (keypath, value, options) {
  ...
  return animate(this.blcktive, model, value, options);
};
...
Context__proto__.observe = function observe (keypath, callback, options) {
  ...
  return this.blcktive.observe(keypath, callback, options);
};
...
Context__proto__.observeOnce = function observeOnce (keypath, callback, options) {
  ...
  if (isObject(keypath)) { options = callback || {}; }
  options.fragment = this.fragment;
  return this.blcktive.observeOnce(keypath, callback, options);
};
...
Context__proto__.readLink = function readLink (keypath, options) {
  return this.blcktive.readLink(this.resolve(keypath), options);
};

Context__proto__.resolve = function resolve (path, blcktive) {
  var ref = findModel(this, path);
    var model = ref.model;
    var instance = ref.instance;
  return model ? model.getKeypath(blcktive || instance) : path;
};
...
Context__proto__.update = function update (keypath, options) {
  return update$1(this.blcktive, findModel(this, keypath).model, options);
};
...
Context__proto__.getBindingPath = function getBindingPath (blcktive) {
  var ref = this.getBindingModel(this);
    var model = ref.model;
    var instance = ref.instance;
  if (model) { return model.getKeypath(blcktive || instance); }
};
...
Context__proto__.getBindingModel = function getBindingModel (ctx) {
  var el = ctx.element;
  return { model: el.binding && el.binding.model, instance: el.up.blcktive };
};
...
Context.forblcktive = getblcktiveContext;
...
function findModel(ctx, path) {
  ...
  return { model: resolveReference(frag, path), instance: frag.blcktive };
}
...
function blcktive$fire(eventName) {
  var args = [], len = arguments.length - 1;
  while ( len-- > 0 ) args[ len ] = arguments[ len + 1 ];
  ...
  if (args[0] instanceof Context) {
    ...
    ctx = Context.forblcktive(this, args.shift());
  } else {
    ctx = Context.forblcktive(this);
  }

  return fireEvent(this, eventName, ctx, args);
}

function blcktive$get(keypath, opts) {
...
function getContext$1(node) {
  ...
  if (node) {
    if (node._blcktive) {
      return node._blcktive.proxy.getContext();
    } else if ((instances = node.__blcktive_instances__)) {
      if (instances.length === 1) { return getblcktiveContext(instances[0]); }
    } else { return getContext$1(node.parentNode); }
  }
}

function getContext$2(node, options) {
  if (!node) { return getblcktiveContext(this); }
...
if (!svg) {
  ...
  createElement = function (type, ns, is) {
    if (ns && ns !== html) {
      throw "This browser does not support namespaces other than http://www.w3.org/1999/xhtml. The most likely cause of this error is that you're trying to render SVG in an older browser. See http://blcktive.js.org/support/#svgs for more information";
    }
...
function blcktive$insert(target, anchor) {
  if (!this.fragment.rendered) {
    // TODO create, and link to, documentation explaining this
    throw new Error(
      'The API has changed - you must call `blcktive.render(target[, anchor])` to render your blcktive instance. Once rendered you can use `blcktive.insert()`.'
    );
  }
  ...
  (target.__blcktive_instances__ || (target.__blcktive_instances__ = [])).push(this);
  this.isDetached = false;

  fireInsertHook(this);
}

function fireInsertHook(blcktive) {
  hooks.insert.fire(blcktive);

  blcktive.findAllComponents('*').forEach(function (child) {
...
function link(there, here, options) {
  var model;
  var target = (options && (options.blcktive || options.instance)) || this;
...
var Observer = function Observer(blcktive, model, callback, options) {
  this.context = options.context || blcktive;
  this.callback = callback;
  this.blcktive = blcktive;
  ...
  if (isFunction(options.old)) {
    this.oldContext = create(blcktive);
...
Observer__proto__.cancel = function cancel () {
  ...
  removeFromArray(this.blcktive._observers, this);
};
...
var PatternObserver = function PatternObserver(blcktive, baseModel, keys$$1, callback, options) {
  var this$1 = this;
  ...
  var baseKeypath = (this.baseKeypath = baseModel.getKeypath(blcktive));
  ...
  models.forEach(function (model) {
    this$1.newValues[model.getKeypath(this$1.blcktive)] = model.get();
  });
  ...
  if (!this.dirty || this.changed.length) {
    ...
    if (!this.changed.length) {
      this.baseModel.findMatches(this.keys).forEach(function (model) {
        var keypath = model.getKeypath(this$1.blcktive);
...
var ArrayObserver = function ArrayObserver(blcktive, model, callback, options) {
  this.blcktive = blcktive;
  this.model = model;
...
function observe(keypath, callback, options) {
  ...
  // add observers to the blcktive instance, so they can be
  // cancelled on blcktive.teardown()
...
function createObserver(blcktive, keypath, callback, options) {
  var keys$$1 = splitKeypath(keypath);
  var wildcardIndex = keys$$1.indexOf('*');
  if (!~wildcardIndex) { wildcardIndex = keys$$1.indexOf('**'); }

  options.fragment = options.fragment || blcktive.fragment;

  var model;
  if (!options.fragment) {
    model = blcktive.viewmodel.joinKey(keys$$1[0]);
  } else {
  ...
  if (!model) { model = blcktive.viewmodel.joinKey(keys$$1[0]); }

  if (!~wildcardIndex) {
    model = model.joinAll(keys$$1.slice(1));
    if (options.array) {
      return new ArrayObserver(blcktive, model, callback, options);
    } else {
      return new Observer(blcktive, model, callback, options);
    }
  } else {
    ...
    return new PatternObserver(blcktive, model, keys$$1.slice(wildcardIndex), callback, options);
  }
}
...
function blcktive$off(eventName, callback) {
  var this$1 = this;
...
function blcktive$on(eventName, callback) {
  var this$1 = this;
...
function blcktive$once(eventName, handler) {
...
function readLink(keypath, options) {
  ...
  if (this.viewmodel.has(path[0])) {
    ...
    if (model) { return { blcktive: model.root.blcktive, keypath: model.getKeypath() }; }
  }
}

var PREFIX = '/* blcktive.js component styles */';
...
function getStyle(id) {
  return doc && (doc.querySelector(("[data-blcktive-css=\"" + id + "\"]")) || makeStyle(id));
}
...
function makeStyle(id) {
  if (doc) {
    ...
    el.type = 'text/css';
    el.setAttribute('data-blcktive-css', id || '');
...
var dataRvcGuidPattern = /\[data-blcktive-css~="\{[a-z0-9-]+\}"]/g;
...
function transformCss(css, id) {
  var dataAttr = "[data-blcktive-css~=\"{" + id + "}\"]";
...
function validate(data) {
  ...
  if (data && data.constructor !== Object) {
    if (isFunction(data)) {
      // TODO do we need to support this in the new blcktive() case?
...
var dataConfigurator = {
      ...
      for (key in options.data) {
        ...
        if (value && isObjectType(value)) {
          if (isObject(value) || isArray(value)) {
            warnIfDebug("Passing a `data` option with object and array properties to blcktive.extend() is discouraged, as mutating them is likely to cause bugs. Consider using a data function instead:\n\n  // this...\n  data: function () {\n    return {\n      myObject: {}\n    };\n  })\n\n  // instead of this:\n  data: {\n    myObject: {}\n  }");
          }
        }
      }
    }
  ...
  init: function (Parent, blcktive, options) {
    var result = combine$1(Parent.prototype.data, options.data);

    if (isFunction(result)) { result = result.call(blcktive); }

    // bind functions to the blcktive instance at the top level,
    // unless it's a non-POJO (in which case alarm bells should ring)
    if (result && result.constructor === Object) {
      for (var prop in result) {
        if (isFunction(result[prop])) {
          var value = result[prop];
          result[prop] = bind(value, blcktive);
  ...
  reset: function reset(blcktive) {
    var result = this.init(blcktive.constructor, blcktive, blcktive.viewmodel);
    blcktive.viewmodel.root.set(result);
    return true;
  }
};
...
function combine$1(parentValue, childValue) {
  ...
  // the default data object on blcktive or a component.
  // then blcktive.set() ends up setting on the prototype!
...
var templateConfigurator = {
  ...
  init: function init(Parent, blcktive, options) {
    // TODO because of prototypal inheritance, we might just be able to use
    // blcktive.template, and not bother passing through the Parent object.
    ...
    if (isFunction(template)) {
      var fn = template;
      template = getDynamicTemplate(blcktive, fn);

      blcktive._config.template = {
        fn: fn,
        result: template
      };
    }

    template = parseTemplate(template, blcktive);

    // TODO the naming of this is confusing - blcktive.template refers to [...],
    // but Component.prototype.template refers to {v:1,t:[],p:[]}...
    // it's unnecessary, because the developer never needs to access
    // blcktive.template
    blcktive.template = template.t;

    if (template.p) {
      extendPartials(blcktive.partials, template.p);
    }
  },

  reset: function reset(blcktive) {
    var result = resetValue(blcktive);

    if (result) {
      var parsed = parseTemplate(result, blcktive);

      blcktive.template = parsed.t;
      extendPartials(blcktive.partials, parsed.p, true);
...
function resetValue(blcktive) {
  var initial = blcktive._config.template;
  ...
  var result = getDynamicTemplate(blcktive, initial.fn);
...
function getDynamicTemplate(blcktive, fn) {
  return fn.call(blcktive, {
    fromId: parser.fromId,
    isParsed: parser.isParsed,
    parse: function parse(template, options) {
      if ( options === void 0 ) options = parser.getParseOptions(blcktive);
...
function parseTemplate(template, blcktive) {
  if (isString(template)) {
    // parse will validate and add expression functions
    template = parseAsString(template, blcktive);
  } else {
...
function parseAsString(template, blcktive) {
  // ID of an element containing the template?
  if (template[0] === '#') {
    template = parser.fromId(template);
  }

  return parser.parseFor(template, blcktive);
}
...
function validate$1(template) {
  ...
  if (template == undefined) {
    ...
    throw new Error(
      ("Mismatched template version (expected " + TEMPLATE_VERSION + ", got " + (template.v) + ") Please ensure you are using the latest version of blcktive.js in your build process as well as in your app")
    );
  }
}
...
Registry__proto__.reset = function reset (blcktive) {
  var registry = blcktive[this.name];
  var changed = false;
...
var config = {
  ...
  init: function (Parent, blcktive, options) { return configure('init', Parent, blcktive, options); },
  reset: function (blcktive) { return order.filter(function (c) { return c.reset && c.reset(blcktive); }).map(function (c) { return c.name; }); }
};
  ...
  for (var key in options) {
    if (hasOwn(isStandardKey, key)) {
      ...
      if (key !== 'el' && isFunction(value)) {
        warnIfDebug(
          (key + " is a blcktive option that does not expect a function and will be ignored"),
    ...
    if (!isBlacklisted[key] && hasOwn(options, key)) {
      ...
      if (isFunction(member)) {
        if (
          ...
          warnIfDebug(
            ("Overriding blcktive prototype function '" + key + "' without calling the '" + _super + "' method can be very dangerous.")
          );
        }
...
var Item = function Item(options) {
  this.up = options.up;
  this.blcktive = options.up.blcktive;
...
function updateMultipleSelectValue(reset) {
  ...
  if (reset) {
    ...
    while (i--) {
      ...
      var optionValue = option._blcktive ? option._blcktive.value : option.value; // options inserted via a triple don't have _blcktive
...
function updateSelectValue(reset) {
  ...
  if (!this.locked) {
    ...
    this.node._blcktive.value = value;
...
function updateRadioValue(reset) {
  ...
  node.value = this.node._blcktive.value = value;
...
function updateValue(reset) {
  if (!this.locked) {
    if (reset) {
      ...
      this.node.value = this.node._blcktive.value = null;
    } else {
      var value = this.getValue();

      this.node.value = this.node._blcktive.value = value;
...
var Attribute = (function (Item) {
  function Attribute(options) {
    ...
    this.blcktive = this.up.blcktive;
  ...
  Attribute__proto__.render = function render () {
    ...
    if (!node.namespaceURI || node.namespaceURI === namespaces.html) {
      ...
      if (this.propertyName === 'value') {
        node._blcktive.value = this.value;
      }
    }
...
function blcktive$teardown() {
  var this$1 = this;

  if (this.torndown) {
    warnIfDebug('blcktive.teardown() was called on a blcktive instance that was already torn down');
...
function teardown$1(instance, getPromise) {
  ...
  if (instance.el && instance.el.__blcktive_instances__) {
    removeFromArray(instance.el.__blcktive_instances__, instance);
  }
...
var blcktiveModel = (function (SharedModel) {
  function blcktiveModel(blcktive) {
    SharedModel.call(this, blcktive, '@this');
    this.blcktive = blcktive;
  }

  if ( SharedModel ) blcktiveModel.__proto__ = SharedModel;
  var blcktiveModel__proto__ = blcktiveModel.prototype = Object.create( SharedModel && SharedModel.prototype );
  blcktiveModel__proto__.constructor = blcktiveModel;

  blcktiveModel__proto__.joinKey = function joinKey (key) {
    var model = SharedModel.prototype.joinKey.call(this, key);

    if ((key === 'root' || key === 'parent') && !model.isLink) { return initLink(model, key); }
    else if (key === 'data') { return this.blcktive.viewmodel; }
    else if (key === 'cssData') { return this.blcktive.constructor._cssModel; }

    return model;
  };

  return blcktiveModel;
}(SharedModel));
...
function initLink(model, key) {
  model.applyValue = function(value) {
    ...
    if (value && value.viewmodel) {
      this.link(value.viewmodel.getblcktiveModel(), key);
  ...
  model.applyValue(model.parent.blcktive[key], key);
...
var specialModels = {
  ...
  '@style': function _style(root) {
    return root.getblcktiveModel().joinKey('cssData');
  },
...
var RootModel = (function (Model) {
  function RootModel(options) {
    ...
    this.isRoot = true;
    this.root = this;
    this.blcktive = options.blcktive; // TODO sever this link
  ...
  RootModel__proto__.getHelpers = function getHelpers () {
    if (!this.helpers) { this.helpers = new SharedModel(this.blcktive.helpers, 'helpers', this.blcktive); }
    return this.helpers;
  };
  ...
  RootModel__proto__.getblcktiveModel = function getblcktiveModel () {
    return this.blcktiveModel || (this.blcktiveModel = new blcktiveModel(this.blcktive));
  };
  ...
  RootModel__proto__.teardown = function teardown () {
    ...
    this.blcktiveModel && this.blcktiveModel.teardown();
  };
...
function construct(blcktive, options) {
  if (blcktive.DEBUG) { welcome(); }

  initialiseProperties(blcktive);
  handleAttributes(blcktive);

  // set up event subscribers
  subscribe(blcktive, options, 'on');

  // if there's not a delegation setting, inherit from parent if it's not default
  if (
    !hasOwn(options, 'delegate') &&
    blcktive.parent &&
    blcktive.parent.delegate !== blcktive.delegate
  ) {
    blcktive.delegate = false;
  }

  // plugins that need to run at construct
  if (isArray(options.use)) {
    blcktive.use.apply(blcktive, options.use.filter(function (p) { return p.construct; }));
  }

  hooks.construct.fire(blcktive, options);
  if (options.onconstruct) { options.onconstruct.call(blcktive, getblcktiveContext(blcktive), options); }

  // Add registries
  var i = registryNames$1.length;
  while (i--) {
    var name = registryNames$1[i];
    blcktive[name] = assign(create(blcktive.constructor[name] || null), options[name]);
  }

  i = protoRegistries.length;
  while (i--) {
    var name$1 = protoRegistries[i];
    blcktive[name$1] = assign(create(blcktive.constructor.prototype[name$1]), options[name$1]);
  }

  if (blcktive._attributePartial) {
    blcktive.partials['extra-attributes'] = blcktive._attributePartial;
    delete blcktive._attributePartial;
  }

  // Create a viewmodel
  var viewmodel = new RootModel({
    adapt: getAdaptors(blcktive, blcktive.adapt, options),
    data: dataConfigurator.init(blcktive.constructor, blcktive, options),
    blcktive: blcktive
  });

  // once resolved, share the adaptors array between the root model and instance
  blcktive.adapt = viewmodel.adaptors;

  blcktive.viewmodel = viewmodel;

  for (var k in blcktive.computed) {
    compute.call(blcktive, k, blcktive.computed[k]);
  }
}

function getAdaptors(blcktive, protoAdapt, options) {
  protoAdapt = protoAdapt.map(lookup);
  var adapt = ensureArray(options.adapt).map(lookup);

  var srcs = [protoAdapt, adapt];
  if (blcktive.parent && !blcktive.isolated) {
    srcs.push(blcktive.parent.viewmodel.adaptors);
  }
  ...
  function lookup(adaptor) {
    if (isString(adaptor)) {
      adaptor = findInViewHierarchy('adaptors', blcktive, adaptor);
...
function initialiseProperties(blcktive) {
  // Generate a unique identifier, for places where you'd use a weak map if it
  // existed
  blcktive._guid = 'r-' + uid++;

  // events
  blcktive._subs = create(null);
  blcktive._nsSubs = 0;
  ...
  // external children
  blcktive._children = [];
  blcktive._children.byName = {};
  blcktive.children = blcktive._children;

  if (!blcktive.component) {
    blcktive.root = blcktive;
    blcktive.parent = blcktive.container = null; // TODO container still applicable?
  }
}

function handleAttributes(blcktive) {
  var component = blcktive.component;
  var attributes = blcktive.constructor.attributes;
  ...
  if (attributes && component) {
    ...
    blcktive._attributePartial = partial;
  }
}
...
var Component = (function (Item) {
  function Component(options, ComponentConstructor) {
    ...
    if (this.isAnchor) {
      ...
      instance.parent = this.up.blcktive;
      ...
      // for any blcktive instance, but for now, just
      // for components and just for blcktive...
  ...
  Component__proto__.getContext = function getContext () {
    ...
    return getblcktiveContext.apply(null, assigns);
  };
...
var ComputationChild = (function (Model) {
  function ComputationChild(parent, key) {
    ...
    this.isReadonly = !this.root.blcktive.syncComputedChildren;
    this.dirty = true;
...
var Computation = (function (Model) {
  ...
  Computation__proto__.getContext = function getContext () {
    return this.parent.isRoot ? this.root.blcktive : this.parent.get(false, noVirtual);
  };
  ...
  Computation__proto__.getValue = function getValue () {
    ...
    try {
      result = this.signature.getter.call(this.root.blcktive, this.getContext(), this.getKeypath());
    } catch (err) {
      warnIfDebug(("Failed to compute " + (this.getKeypath()) + ": " + (err.message || err)));

      // TODO this is all well and good in Chrome, but...
      // ...also, should encapsulate this stuff better, and only
      // show it if blcktive.DEBUG
      if (hasConsole) {
...
var ExpressionProxy = (function (Model) {
  function ExpressionProxy(fragment, template) {
    ...
    Model.call(this, fragment.blcktive.viewmodel, null);
    ...
    this.fn =
      fragment.blcktive.allowExpressions === false
        ? noop
  ...
  ExpressionProxy__proto__.getValue = function getValue () {
    ...
    try {
      ...
      result = this.fn.apply(this.fragment.blcktive, params);
    } catch (err) {
      warnIfDebug(("Failed to compute " + (this.getKeypath()) + ": " + (err.message || err)));
    }
...
var Decorator = function Decorator(options) {
  ...
  this.blcktive = this.up.blcktive || this.owner.blcktive;
...
Decorator__proto__.render = function render () {
  ...
  runloop.scheduleTask(function () {
    ...
    this$1.handle = fn.apply(blcktive, [this$1.node].concat(args));
...
Decorator__proto__.update = function update () {
  ...
  if (instance) {
    if (!instance.update) {
      ...
      instance.update.apply(this.blcktive, args);
    }
  }
};
...
var Binding = function Binding(element, name) {
  ...
  this.blcktive = element.blcktive;
...
Binding__proto__.render = function render () {
  ...
  this.node._blcktive.binding = this;
...
function handleDomEvent() {
  this._blcktive.binding.handleChange();
}
...
var CheckboxNameBinding = (function (Binding) {
  function CheckboxNameBinding(element) {
    ...
    this.checkboxName = true; // so that blcktive.updateModel() knows what to do with this
...
function handleBlur() {
  ...
  var value = this._blcktive.binding.model.get();
...
function handleDelay(delay) {
  ...
  return function() {
    ...
    timeout = setTimeout(function () {
      var binding = this$1._blcktive.binding;
...
var MultipleSelectBinding = (function (Binding) {
  ...
  MultipleSelectBinding__proto__.getValue = function getValue () {
    ...
    for (var i = 0; i < len; i += 1) {
      ...
      if (option.selected) {
        var optionValue = option._blcktive ? option._blcktive.value : option.value;
  ...
  MultipleSelectBinding__proto__.setFromNode = function setFromNode (node) {
    ...
    while (i--) {
      ...
      result[i] = option._blcktive ? option._blcktive.value : option.value;
    }
...
var RadioBinding = (function (Binding) {
  function RadioBinding(element) {
    ...
    this.siblings = getSiblings(this.blcktive._guid + this.element.getAttribute('name'));
...
var SingleSelectBinding = (function (Binding) {
  ...
  SingleSelectBinding__proto__.getValue = function getValue () {
    ...
    for (i = 0; i < len; i += 1) {
      ...
      if (options[i].selected && !options[i].disabled) {
        return option._blcktive ? option._blcktive.value : option.value;
      }
    }
  };
  ...
  SingleSelectBinding__proto__.setFromNode = function setFromNode (node) {
    ...
    this.model.set(option._blcktive ? option._blcktive.value : option.value);
  };
...
function selectBinding(element) {
  ...
  if (name === 'input') {
    ...
    if (type === 'checkbox') {
      ...
      //
      // See https://github.com/ungtb10d/blcktive/issues/1749
...
var Element = (function (ContainerItem) {
  ...
  Element__proto__.createTwowayBinding = function createTwowayBinding () {
    if ('twoway' in this ? this.twoway : this.blcktive.twoway) {
  ...
  Element__proto__.render = function render (target, occupants) {
    ...
    // a data-blcktive-css attribute, for CSS encapsulation
    if (this.up.cssIds) {
      node.setAttribute('data-blcktive-css', this.up.cssIds.map(function (x) { return ("{" + x + "}"); }).join(' '));
    }
  ...
  Element__proto__.toString = function toString () {
    ...
    if (this.up.cssIds) {
      attrs += " data-blcktive-css=\"" + (this.up.cssIds.map(function (x) { return ("{" + x + "}"); }).join(' ')) + "\"";
    }
...
function getNamespace(element) {
  ...
  return element.blcktive.el.namespaceURI;
}
...
function delegateHandler(ev) {
  ...
  var endEl = end._blcktive && end._blcktive.proxy;
  var node = ev.target;
  var bubble = true;
  var listeners;
  var prevented = false;
  ...
  // starting with the origin node, walk up the DOM looking for blcktive nodes with a matching event listener
  while (bubble && node && node !== end) {
    var proxy = node._blcktive && node._blcktive.proxy;
...
function handleReset() {
  var element = this._blcktive.proxy;
...
var blcktiveEvent = function blcktiveEvent(component, name) {
  this.component = component;
  this.name = name;
  this.handler = null;
};
var blcktiveEvent__proto__ = blcktiveEvent.prototype;

blcktiveEvent__proto__.bind = function bind (directive) {
  var blcktive = this.component.instance;

  this.handler = blcktive.on(this.name, function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];
...
blcktiveEvent__proto__.render = function render () {};

blcktiveEvent__proto__.unbind = function unbind () {
  this.handler.cancel();
};

blcktiveEvent__proto__.unrender = function unrender () {};
...
EventDirective__proto__.bind = function bind () {
  ...
  if (this.element.type === COMPONENT || this.element.type === ANCHOR) {
    this.template.n.forEach(function (n) {
      this$1.events.push(new blcktiveEvent(this$1.element, n));
    });
  } else {
    var args;
    ...
    this.template.n.forEach(function (n) {
      var fn = findInViewHierarchy('events', this$1.blcktive, n);
      if (fn) {
...
EventDirective__proto__.fire = function fire (event, args) {
  ...
  if (this.fn) {
    ...
    var blcktive = this.blcktive;
    var oldEvent = blcktive.event;

    blcktive.event = context;
    var returned = this.fn.apply(blcktive, values);
    ...
    if (result === false) {
      ...
      result = fireEvent(this.blcktive, result.shift(), context, result);
    }

    blcktive.event = oldEvent;

    return result;
  } else {
    return fireEvent(this.blcktive, this.action, context, args);
  }
};
...
var ReferenceExpressionProxy = (function (LinkModel) {
  function ReferenceExpressionProxy(fragment, template) {
    ...
    this.root = fragment.blcktive.viewmodel;
    this.template = template;
    this.rootLink = true;
    this.template = template;
    this.fragment = fragment;
...
var Option = (function (Element) {
  ...
  Option__proto__.render = function render (target, occupants) {
    ...
    if (!this.attributeByName.value) {
      this.node._blcktive.value = this.getAttribute('value');
    }
  };
...
function getPartialTemplate(blcktive, name, up) {
  // If the partial in instance or view heirarchy instances, great
  var partial = getPartialFromRegistry(blcktive, name, up || {});
  if (partial) { return partial; }

  // Does it exist on the page as a script tag?
  partial = parser.fromId(name, { noThrow: true });
  if (partial) {
    // parse and register to this blcktive instance
    var parsed = parser.parseFor(partial, blcktive);

    // register extra partials on the blcktive instance if they don't already exist
    if (parsed.p) { fillGaps(blcktive.partials, parsed.p); }

    // register (and return main partial if there are others in the template)
    return (blcktive.partials[name] = parsed.t);
  }
}

function getPartialFromRegistry(blcktive, name, up) {
  // if there was an instance up-hierarchy, cool
  var partial = findParentPartial(name, up.owner);
  if (partial) { return partial; }

  // find first instance in the blcktive or view hierarchy that has this partial
  var instance = findInstance('partials', blcktive, name);
  ...
  if (isFunction(partial)) {
    ...
    partial = fn.call(blcktive, parser);
  }
  ...
  if (!parser.isParsed(partial)) {
    // use the parseOptions of the blcktive instance on which it was found
    var parsed = parser.parseFor(partial, instance);

    // Partials cannot contain nested partials!
    // TODO add a test for this
    if (parsed.p) {
      warnIfDebug('Partials ({{>%s}}) cannot contain nested inline partials', name, { blcktive: blcktive });
    }
...
function findOwner(blcktive, key) {
  return hasOwn(blcktive.partials, key) ? blcktive : findConstructor(blcktive.constructor, key);
}
...
assign(proto$6, {
  ...
  bind: function bind() {
    ...
    if (this.yielder) {
      ...
      this.container = this.up.blcktive;
  ...
  resetTemplate: function resetTemplate() {
    ...
    if (this.refName) {
      this.partial = getPartialTemplate(this.blcktive, this.refName, this.up);
    }
...
function createFragment(self, partial) {
  ...
  if (self.yielder) { options.blcktive = self.container.parent; }
...
function partialFromValue(self, value, okToParse) {
  ...
  if (isArray(tpl)) {
    ...
    if (isArray(tpl.t)) { self.partial = tpl.t; }
      ...
      { self.partial = parsePartial(tpl.template, tpl.template, self.blcktive).t; }
  } else if (isFunction(tpl) && tpl.styleSet) {
    self.fn = tpl;
    if (self.fragment) { self.fragment.cssIds = tpl._cssIds; }
  } else if (tpl != null) {
    tpl = getPartialTemplate(self.blcktive, '' + tpl, self.containerFragment || self.up);
    if (tpl) {
      self.name = value;
      if (tpl.styleSet) {
        self.fn = tpl;
      ...
      self.partial = parsePartial('' + value, '' + value, self.blcktive).t;
    } else {
      self.name = value;
    }
  }
...
function initMacro(self) {
  ...
  self.initing = 1;
  self.proxy = fn.call(self.blcktive, handle, handle.attributes) || {};
...
function parsePartial(name, partial, blcktive) {
  var parsed;

  try {
    parsed = parser.parse(partial, parser.getParseOptions(blcktive));
  } catch (e) {
...
var RepeatedFragment = function RepeatedFragment(options) {
  ...
  this.up = this;
  this.owner = options.owner;
  this.blcktive = this.parent.blcktive;
  this.delegate =
    this.blcktive.delegate !== false && (this.parent.delegate || findDelegate(this.parent));
...
function findDelegate(start) {
  ...
  out: while (frag) {
    ...
    while (!el && frag) {
      ...
      if (frag.owner.blcktive && frag.owner.blcktive.delegate === false) { break out; }
...
function swizzleFragment(section, fragment, key, idx) {
  ...
  if (fragment.rootModel) {
    ...
    fragment.rootModel.applyValue(model.getKeypath(fragment.blcktive.root));
  }
...
var Select = (function (Element) {
  ...
  Select__proto__.sync = function sync () {
    ...
    if (selectValue !== undefined) {
      ...
      options.forEach(function (o) {
        var optionValue = o._blcktive ? o._blcktive.value : o.value;
    ...
    setTimeout(function () {
      ...
      function checkComplete() {
        if (jsTransitionsComplete && cssTransitionsComplete) {
          ...
          t.blcktive.fire(t.name + ':end', t.node, t.isIntro);
          resolve();
        }
      }
      ...
      setTimeout(function () {
        ...
        while (i--) {
          ...
          if (!cssTiming || !CSS_TRANSITIONS_ENABLED || cannotUseCssTransitions[hash]) {
            ...
            if (index === -1) {
              warnIfDebug(
                'Something very strange happened with transitions. Please raise an issue at https://github.com/ungtb10d/blcktive/issues - thanks!',
                { node: t.node }
              );
            } else {
              changedProperties.splice(index, 1);
        ...
        if (propertiesToTransitionInJs.length) {
          ...
          if (isString(options.easing)) {
            easing = t.blcktive.easing[options.easing];
...
var Transition = function Transition(options) {
  this.owner = options.owner || options.up.owner || findElement(options.up);
  this.element = this.owner.attributeByName ? this.owner : findElement(options.up);
  this.blcktive = this.owner.blcktive;
  this.template = options.template;
  this.up = options.up;
  this.options = options;
  this.onComplete = [];
...
Transition__proto__.bind = function bind () {
  ...
  var blcktive = this.owner.blcktive;
  ...
  if (isFunction(this.name)) {
    ...
    this._fn = findInViewHierarchy('transitions', blcktive, this.name);
  }

  if (!this._fn) {
    warnOnceIfDebug(missingPlugin(this.name, 'transition'), { blcktive: blcktive });
  }
...
Transition__proto__.getParams = function getParams () {
  ...
  if (this.fn) {
    ...
    return this.fn.apply(this.blcktive, values);
  }
};
...
Transition__proto__.shouldFire = function shouldFire (type) {
  if (!this.blcktive.transitionsEnabled) { return false; }

  // check for noIntro and noOutro cases, which only apply when the owner blcktive is rendering and unrendering, respectively
  if (type === 'intro' && this.blcktive.rendering && nearestProp('noIntro', this.blcktive, true))
    { return false; }
  if (type === 'outro' && this.blcktive.unrendering && nearestProp('noOutro', this.blcktive, false))
    { return false; }
  ...
  if (params && params[0] && isObject(params[0]) && 'nested' in params[0]) {
    ...
    if (nearestProp('nestedTransitions', this.blcktive) !== false) { return true; }
  }
...
Transition__proto__.start = function start () {
  ...
  var promise = this._fn.apply(this.blcktive, [this].concat(args));
...
function nearestProp(prop, blcktive, rendering) {
  var instance = blcktive;
  while (instance) {
    if (
    ...
    instance = instance.component && instance.component.blcktive;
  }

  return blcktive[prop];
}
...
function getComponentConstructor(blcktive, name) {
  var instance = findInstance('components', blcktive, name);
  var Component;
    ...
    if (Component && !Component.isInstance) {
      if (Component.default && Component.default.isInstance) { Component = Component.default; }
        ...
        if (isString(Component)) {
          // allow string lookup
          Component = getComponentConstructor(blcktive, Component);
        }
...
function asyncProxy(promise, options) {
  ...
  var opts = assign({}, options, {
    ...
    macro: function macro(handle) {
      ...
      promise.then(
        function (cmp) {
          options.up.blcktive.components[name] = cmp;
...
function createItem(options) {
  ...
  if (type === ELEMENT) {
    ...
    ctor = findInstance('partials', options.up.blcktive, name);
    if (ctor) {
    ...
    ctor = getComponentConstructor(options.up.blcktive, name);
    if (ctor) {
...
var Fragment = function Fragment(options) {
  ...
  this.blcktive = options.blcktive || (this.isRoot ? options.owner : this.parent.blcktive);

  this.componentParent =
    this.isRoot && this.blcktive.component
      ? this.blcktive.component.up
      : this.owner.containerFragment || null;
  if (!this.isRoot || this.blcktive.delegate) {
...
Fragment__proto__.bubble = function bubble () {
  ...
  if (!this.dirty) {
    ...
    if (this.isRoot) {
      ...
      if (this.blcktive.component) {
        this.blcktive.component.bubble();
      } else if (this.bound) {
...
Fragment__proto__.findContext = function findContext () {
  ...
  if (!base || !base.context) { return this.blcktive.viewmodel; }
...
Fragment__proto__.findNextNode = function findNextNode (item) {
  ...
  if (this.isRoot) {
    if (this.blcktive.component) {
      return this.blcktive.component.up.findNextNode(this.blcktive.component);
    }

    // TODO possible edge case with other content
    // appended to this.blcktive.el?
    return null;
  }
...
Fragment__proto__.findParentNode = function findParentNode () {
  ...
  do {
    ...
    if (fragment.isRoot && !fragment.blcktive.component) {
      // TODO encapsulate check
      return fragment.blcktive.el;
    }
...
Fragment__proto__.rebound = function rebound (update$$1) {
  ...
  if (update$$1) {
    if (this.rootModel) { this.rootModel.applyValue(this.context.getKeypath(this.blcktive.root)); }
...
Fragment__proto__.valueOf = function valueOf () {
  ...
  if (this.dirtyValue) {
    var values = {};
    var source = processItems(this.items, values, this.blcktive._guid);
  ...
  if (root) {
    if (!this.rootModel) {
      this.rootModel = new KeyModel(
        this.context.getKeypath(this.blcktive.root),
        this.context,
        this.blcktive.root
      );
...
function initialise(blcktive, userOptions, options) {
  // initialize settable computeds
  var computed = blcktive.viewmodel.computed;
  if (computed) {
    for (var k in computed) {
      if (k in blcktive.viewmodel.value && computed[k] && !computed[k].isReadonly) {
        computed[k].set(blcktive.viewmodel.value[k]);
      }
    }
  }

  // init config from Parent and options
  config.init(blcktive.constructor, blcktive, userOptions);

  // call any passed in plugins
  if (isArray(userOptions.use))
    { blcktive.use.apply(blcktive, userOptions.use.filter(function (p) { return !p.construct; })); }

  hooks.config.fire(blcktive);

  hooks.init.begin(blcktive);

  var fragment = (blcktive.fragment = createFragment$1(blcktive, options));
  if (fragment) { fragment.bind(blcktive.viewmodel); }

  hooks.init.end(blcktive);

  // general config done, set up observers
  subscribe(blcktive, userOptions, 'observe');

  if (fragment) {
    // render automatically ( if `el` is specified )
    var el = (blcktive.el = blcktive.target = getElement(blcktive.el || blcktive.target));
    if (el && !blcktive.component) {
      var promise = blcktive.render(el, blcktive.append);

      if (blcktive.DEBUG_PROMISES) {
        promise.catch(function (err) {
          warnOnceIfDebug(
            'Promise debugging is enabled, to help solve errors that happen asynchronously. Some browsers will log unhandled promise rejections, in which case you can safely disable promise debugging:\n  blcktive.DEBUG_PROMISES = false;'
          );
          warnIfDebug('An error happened during rendering', { blcktive: blcktive });
          logIfDebug(err);
...
function createFragment$1(blcktive, options) {
  if ( options === void 0 ) options = {};

  if (blcktive.template) {
    var cssIds = [].concat(blcktive.constructor._cssIds || [], options.cssIds || []);

    return new Fragment({
      owner: blcktive,
      template: blcktive.template,
      cssIds: cssIds
    });
  }
}

function render$1(blcktive, target, anchor, occupants) {
  // set a flag to let any transitions know that this instance is currently rendering
  blcktive.rendering = true;

  var promise = runloop.start();
  runloop.scheduleTask(function () { return hooks.render.fire(blcktive); }, true);

  if (blcktive.fragment.rendered) {
    throw new Error(
      'You cannot call blcktive.render() on an already rendered instance! Call blcktive.unrender() first'
    );
  }

  if (blcktive.destroyed) {
    blcktive.destroyed = false;
    blcktive.fragment = createFragment$1(blcktive).bind(blcktive.viewmodel);
  }

  anchor = getElement(anchor) || blcktive.anchor;

  blcktive.el = blcktive.target = target;
  blcktive.anchor = anchor;

  // ensure encapsulated CSS is up-to-date
  if (blcktive.cssId) { applyCSS(); }

  if (target) {
    (target.__blcktive_instances__ || (target.__blcktive_instances__ = [])).push(blcktive);

    if (anchor) {
      var docFrag = doc.createDocumentFragment();
      blcktive.fragment.render(docFrag);
      target.insertBefore(docFrag, anchor);
    } else {
      blcktive.fragment.render(target, occupants);
    }
  }

  runloop.end();
  blcktive.rendering = false;

  return promise.then(function () {
    if (blcktive.torndown) { return; }

    hooks.complete.fire(blcktive);
  });
}

function blcktive$render(target, anchor) {
  if (this.torndown) {
    warnIfDebug('blcktive.render() was called on a blcktive instance that was already torn down');
    return Promise.resolve();
  }
  ...
  if (!this.append && target) {
    ...
    var others = target.__blcktive_instances__;
...
function blcktive$resetTemplate(template) {
...
function blcktive$set(keypath, value, options) {
  var blcktive = this;

  var opts = isObjectType(keypath) ? value : options;

  return set(build(blcktive, keypath, value, opts && opts.isolated), opts);
}
...
function blcktive$subtract(keypath, d, options) {
  var num = isNumber(d) ? -d : -1;
  var opts = isObjectType(d) ? d : options;
  return add(this, keypath, num, opts);
}

function blcktive$toggle(keypath, options) {
  if (!isString(keypath)) {
...
function blcktive$transition(name, node, params) {
  ...
  if (!node || !node._blcktive) {
    fatal(("No node was supplied for transition " + name));
  }

  params = params || {};
  var owner = node._blcktive.proxy;
...
function blcktive$unrender() {
  if (!this.fragment.rendered) {
    warnIfDebug('blcktive.unrender() was called on a blcktive instance that was not rendered');
    return Promise.resolve();
  }
...
function blcktive$updateModel(keypath, cascade) {
  var promise = runloop.start();
...
function use() {
  ...
  plugins.forEach(function (p) {
    p({
      proto: this$1,
      blcktive: this$1.constructor.blcktive,
      instance: this$1
    });
  });
  return this;
...
var proto$9 = {
  add: blcktive$add,
  animate: blcktive$animate,
  attachChild: attachChild,
  compute: blcktive$compute,
  detach: blcktive$detach,
  detachChild: detachChild,
  find: blcktive$find,
  findAll: blcktive$findAll,
  findAllComponents: blcktive$findAllComponents,
  findComponent: blcktive$findComponent,
  findContainer: blcktive$findContainer,
  findParent: blcktive$findParent,
  fire: blcktive$fire,
  get: blcktive$get,
  getLocalContext: getLocalContext,
  getContext: getContext$2,
  insert: blcktive$insert,
  link: link,
  observe: observe,
  ...
  push: push,
  readLink: readLink,
  render: blcktive$render,
  reset: blcktive$reset,
  resetPartial: resetPartial,
  resetTemplate: blcktive$resetTemplate,
  reverse: reverse,
  set: blcktive$set,
  shift: shift,
  sort: sort,
  splice: splice,
  subtract: blcktive$subtract,
  teardown: blcktive$teardown,
  toggle: blcktive$toggle,
  toCSS: blcktive$toCSS,
  toCss: blcktive$toCSS,
  toHTML: blcktive$toHTML,
  toHtml: blcktive$toHTML,
  toText: toText,
  transition: blcktive$transition,
  unlink: unlink,
  unrender: blcktive$unrender,
  unshift: unshift,
  update: blcktive$update,
  updateModel: blcktive$updateModel,
  use: use
};
...
function extendOne(Parent, options, Target) {
  ...
  if (options.prototype instanceof blcktive) {
    throw new Error("blcktive no longer supports multiple inheritance.");
  }
...
defineProperties(blcktive, {
  sharedGet: { value: sharedGet },
  sharedSet: { value: sharedSet },
  styleGet: { configurable: true, value: styleGet.bind(blcktive) },
  styleSet: { configurable: true, value: setCSSData.bind(blcktive) },
  addCSS: { configurable: false, value: addStyle.bind(blcktive) },
  hasCSS: { configurable: false, value: hasStyle.bind(blcktive) }
});
...
function blcktive(options) {
  if (!(this instanceof blcktive)) { return new blcktive(options); }
...
// check to see if we're being asked to force blcktive as a global for some weird environments
if (win && !win.blcktive) {
  var opts$1 = '';
  var script =
    document.currentScript ||
    /* istanbul ignore next */ document.querySelector('script[data-blcktive-options]');

  if (script) { opts$1 = script.getAttribute('data-blcktive-options') || ''; }

  /* istanbul ignore next */
  if (~opts$1.indexOf('ForceGlobal')) { win.blcktive = blcktive; }
} else if (win) {
  warn("blcktive already appears to be loaded while loading 1.4.1.");
}

assign(blcktive.prototype, proto$9, defaults);
blcktive.prototype.constructor = blcktive;

// alias prototype as `defaults`
blcktive.defaults = blcktive.prototype;

// share defaults with the parser
shared.defaults = blcktive.defaults;
shared.blcktive = blcktive;
...
defineProperties(blcktive, {
  ...
  // for getting the source blcktive lib from a constructor
  blcktive: { value: blcktive },
...
defineProperty(blcktive, '_cssModel', {
  configurable: true,
  value: new CSSModel(blcktive)
});

defineProperty(blcktive.prototype, 'rendered', {
  get: function get() {
...
//# sourceMappingURL=blcktive.js.map
