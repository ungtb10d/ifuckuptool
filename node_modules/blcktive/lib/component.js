/* eslint-env node */
const { reducePromiseFunctions, blcktive } = require('./util');
...
const help = `
    component
      ...
      Any additional script tags encountered with type text/blcktive or text/html
      ...
      -e,  --extension {ext} - the extension to look for when searching for files in an
           inputdir - defaults to .blcktive.html
...
function readTemplate(string, opts, readFile, allowScript) {
  const tpl = blcktive.parse(string || '', opts);
  const partials = {};
  const script = [];
  const style = [];
  const exprs = tpl.e || {};
  ...
  while (i--) {
    ...
    if (item.t === 7) {
      if (item.e === 'script') {
        ...
        if (id && (type === 'text/blcktive' || type === 'text/html')) {
          if (!src) {
        ...
        if (
          href &&
          (type === 'component' ||
            ((!type || type === 'text/css' || type === 'text/css+blcktive') &&
              (rel === 'blcktive' || rel === 'component')))
        ) {
          const css = { type: type === 'text/css+blcktive' ? 'tpl' : 'css' };
          style.unshift(css);
...
function compileCss(styles, opts) {
  ...
  styles.forEach(style => {
    if (style.type === 'tpl') {
      ...
      style.compiled = `(function () { return this.blcktive({ template: ${stringify(
        blcktive.parse(style.body, styleOpts),
        opts
