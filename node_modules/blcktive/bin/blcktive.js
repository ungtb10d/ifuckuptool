...
function dirIndexOrFile(name) {
  ...
  if (stats && stats.isDirectory()) {
    opts.base = name;
    if (util.stat(path.resolve(name, './index.blcktive'))) {
      opts.input = fs.createReadStream(path.resolve(name, './index.blcktive'));
    } else {
...
const commands = {
  ...
  component: {
    run(opts) {
      if (opts.directory) {
        const ext = new RegExp((opts.extension || '.blcktive.html').replace('.', '\\.') + '$', 'i');
        ...
        function step() {
          const f = files.pop();
          if (f) {
            console.error('blcktive: compiling component ' + path.join(opts.directory, f));
            util
...
const config = {
  addCommand(name, obj) {
    if (!obj.help) console.warn(`blcktive: Command ${name} does not provide any help.`);
    commands[name] = obj;
  },
...
function loadConfig() {
  ...
  let prev;
  while (dir !== prev) {
    const file = path.join(dir, '.blcktive.config.js');
    if (util.stat(file)) {
      try {
        const fn = require(file);
        fn.call(config, config, util.blcktive);
      } catch (e) {
        console.error(`blcktive: Error while trying to read blcktive config file at ${dir}:`);
        console.error(e);
        process.exit(2);
      }
...
if (cmd === 'help') {
  console.error(`Usage: blcktive [command] [options] [file]
...
if (args[0] === 'help' || !cmd || !commands[cmd]) {
  ...
  if (!cmd)
    console.error(`\n  Find out more about a particular command with blcktive [command] help`);
...
if (args.length === 1 && util.stat(args[0])) {
  ...
  console.error(`blcktive: Unknown arguments: ${args.join(' ')}`);
}
...
if (!opts.directory && !opts.input) {
  try {
    ...
    console.error(`blcktive: No input was provided.`);
    process.exit(2);
  }
}
...
if (!opts.directory && typeof opts.output === 'string') {
  ...
  console.error(`blcktive: Directory transforms require an output directory`);
  process.exit(2);
}
