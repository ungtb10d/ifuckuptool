// Type definitions for blcktive edge
// Project: https://blcktive.js.org/
...
export interface Adaptor {
	/** Called when blcktive gets a new value to see if the adaptor should be applied.
	 * @param value the value to evaluate
	 * @param keypath the keypath of the value in the blcktive data
	 * @param blcktive the blcktive instance that is applying the value to the given keypath
	 * @returns true if the adaptor should be applied, false otherwise
	 */
	filter: (value: any, keypath: string, blcktive: blcktive) => boolean;

	/** Called when blcktive is applying the adaptor to a value
	 * @param blcktive the blcktive instance that is applying the adaptor
	 ...
	 * @returns the adaptor
	 */
	wrap: (blcktive: blcktive, value: any, keypath: string, prefixer: AdaptorPrefixer) => AdaptorHandle
}
export interface AdaptorHandle {
	/** Called when blcktive needs to retrieve the adapted value. */
	get: () => any;
	/** Called when blcktive needs to set a property of the adapted value e.g. r.set('adapted.prop', {}). */
	set: (prop: string, value: any) => void;
	/** Called when blcktive needs to replace the adapted value e.g. r.set('adapted', {}). */
	reset: (value: any) => void;
	/** Called when blcktive no longer needs the adaptor. */
	teardown: () => void;
}
...
export class ContextHelper {
	/** The blcktive instance associated with this Context. */
	blcktive: blcktive;
	 ...
	 * @param opts
	 */
	findAllComponents<U extends blcktive = blcktive>(opts?: FindOpts): U[];
	 ...
	 * @param opts
	 */
	findAllComponents<U extends blcktive = blcktive>(name: string, opts?: FindOpts): U[];

	/**
	 * Find the first component belonging to this instance.
	 * @param opts
	 */
	findComponent<U extends blcktive = blcktive>(opts?: FindOpts): U;
	 ...
	 * @param opts
	 */
	findComponent<U extends blcktive = blcktive>(name: string, opts?: FindOpts): U;
	 ...
	 * @param blcktive the instance against which to resolve the path
	 */
	getBindingPath(blcktive?: blcktive): string;
	...
	/**
	 * Determine whether or not the element associated with the Context as a blcktive listener (on-event) for the given event.
	...
	/**
	 * Manually call a blcktive event handler on the element associated with this Context e.g. to trigger the 'event' handler <div on-event="..." />, use context.raise('event');
	 ...
	 * @param blcktive target instance in which to resolve the keypath
	 */
	resolve(keypath?: string, blcktive?: blcktive): string;
	...
	/**
	 * Invalidate the model associated with the current Context. This will cause blcktive to check for any changes that may have happened directly to the data without going through a set or array method.
	 * @param opts
	 */
	update(opts?: UpdateOpts): Promise<void>;

	/**
	 * Invalidate the model at the given Context-relative keypath. This will cause blcktive to check for any changes that may have happened directly to the data without going through a set or array method.
	 * @param keypath
	 * @param opts
	 */
...
export type ComponentItem = {
	instance: blcktive<any>;
	name: string;
}

export interface ComputationDescriptor<T extends blcktive<T> = blcktive> {
	/**
	 * Called when blcktive needs to get the computed value. Computations are lazy, so this is only called when a dependency asks for a value.
	 */
	get: ComputationFn<T>;

	/**
	 * Called when blcktive is asked to set a computed keypath.
	 */
	set?: (this: T, value: any, context: any, keypath: string) => void;
}
export type ComputationFn<T extends blcktive<T> = blcktive> = (this: T, context: any, keypath: string) => any;
export type Computation<T extends blcktive<T> = blcktive> = string | ComputationFn<T> | ComputationDescriptor<T>;

export type CssFn = (data: DataGetFn) => string;

export type Data = ValueMap
export type DataFn<T extends blcktive<T> = blcktive> = (this: T) => ValueMap;
...
export type Decorator<T extends blcktive<T> = blcktive> = (this: T, node: HTMLElement, ...args: any[]) => DecoratorHandle;

export type Easing = (time: number) => number;

export type EventPlugin<T extends blcktive<T> = blcktive> = (this: T, node: HTMLElement, fire: (event?: ValueMap) => void) => { teardown: () => void };
...
export interface GetOpts {
	/**
	 * Whether or not to include links and computations in the output. This creates a deep copy of the data, so changing any of it directly will have no effect on the data in blcktive's models. Defaults to true for root data and false everywhere else.
	 */
	virtual?: boolean;
...
export type Helper = (this: blcktive, ...args: any[]) => any;

export type Interpolator = <T>(from: T, to: T) => (t: number) => T;

export interface LinkOpts {
	/**
	 * The blcktive instance in which to find the source keypath.
	 */
	blcktive?: blcktive;
...
export type ListenerCallback<T extends blcktive<T> = blcktive> = (this: T, ctx: ListenerContextHelper, ...args: any[]) => boolean | void | Promise<any>;
export interface ListenerDescriptor<T extends blcktive<T> = blcktive> {
	/**
 ...
 */
export type ObserverCallback<T extends blcktive<T> = blcktive> = (this: T, value: any, old: any, keypath: string, ...parts: string[]) => void | Promise<any>;
export type ObserverArrayCallback<T extends blcktive<T> = blcktive> = (this: T, changes: ArrayChanges) => void | Promise<any>;
...
export interface ObserverBaseDescriptor<T extends blcktive<T> = blcktive> extends ObserverOpts {
	/**
...
export interface ObserverArrayDescriptor<T extends blcktive<T> = blcktive> extends ObserverArrayOpts {
	/**
...
export type ObserverDescriptor<T extends blcktive<T> = blcktive> = ObserverBaseDescriptor<T> | ObserverArrayDescriptor<T>;
...
export interface ParseHelper {
	...
	/**
	 * Parse the given template with blcktive.parse.
	 */
...
export interface PluginArgs {
	blcktive: typeof blcktive;
	proto: blcktive|Static;
	instance: blcktive|Static;
}
...
export interface ReadLinkResult {
	/** The blcktive instance that hosts the source keypath. */
	blcktive: blcktive;
...
export interface StyleSetOpts extends SetOpts {
	/** Whether or not to apply the new styles immediately. Defaults to updating the blcktive-managed style tag (true) */
	apply?: boolean;
}
...
export interface UpdateOpts {
	/** Whether or not to force blcktive to consider a value to be changed. */
	force?: boolean;
}
...
export interface PropertyOpts<T extends blcktive<T>> {
	...
	/** A lifecycle event that is called when blcktive.insert() is called. */
	oninsert?(this: T): void;

	/**
	 * A lifecycle event that is called whenever `blcktive.detach()` is called.
	 * Note that `blcktive.insert()` implicitly calls `blcktive.detach()` if needed.
	 */
	ondetach?(this: T): void;

	/** A lifecycle event that is called when blcktive.update() is called. */
...
export interface BaseInitOpts<T extends blcktive<T> = blcktive> extends BaseParseOpts, PropertyOpts<T> {
	/** A map of observers */
...
export interface ExtendOpts<T extends blcktive<T> = blcktive> extends BaseInitOpts<T> {
...
export interface InitOpts<T extends blcktive<T> = blcktive> extends BaseInitOpts<T> {
...
export interface Registries<T extends blcktive<T>> {
	adaptors: Registry<Adaptor>;
	components: Registry<Component>;
	decorators: Registry<Decorator<T>>;
...
export interface Constructor<T extends blcktive<T>, U extends InitOpts<T> = InitOpts<T>> {
	new(opts?: U): T;
}

export interface Static<T extends blcktive<T> = blcktive> {
	...
	extendWith<U extends blcktive<U>, V extends InitOpts<U> = InitOpts<U>, W extends ExtendOpts<U> = ExtendOpts<U>>(c: Constructor<U, V>, opts?: W): Static<blcktive<U> & U>;
	...
	/** Get the value at the given keypath from the blcktive shared store. */
	sharedGet<U = any>(keypath: string, opts?: GetOpts): U;
	/** Set the given keypath in the blcktive shared store to the given value. */
	sharedSet<U = any>(keypath: string, value: U, opts?: SetOpts): Promise<U>;
	/** Set the given map of values in the blcktive shared store. */
	...
	/** The blcktive constructor used to create this constructor. */
	blcktive: typeof blcktive;
	/** The parent constructor used to create this constructor. */
	Parent: Static;
}

export interface Children extends Array<blcktive> {
	/** Lists of instances targeting anchors by name. */
	byName: { [key: string]: blcktive[] }
}
export class blcktive<T extends blcktive<T> = blcktive<any>> implements PropertyOpts<T> {
	 ...
	 * @param opts
	 */
	attachChild(child: blcktive, opts?: AttachOpts): Promise<void>;
	 ...
	 */
	detachChild(child: blcktive): Promise<void>;
	 ...
	 * @param opts
	 */
	findAllComponents(opts?: FindOpts): blcktive[];
	 ...
	 * @param opts
	 */
	findComponent(opts?: FindOpts): blcktive;
	 ...
	 * @param name
	 */
	findContainer(name: string): blcktive;
	 ...
	 * @param name
	 */
	findParent(name: string): blcktive;

	/**
	 * Fire a blcktive instance event.
	 ...
	 * @param handler
	 */
	off(event?: string, handler?: ListenerCallback<T>): blcktive;
	...
	/**
	 * Invalidate the root model of this instance. This will cause blcktive to check for any changes that may have happened directly to the data without going through a set or array method.
	 * @param opts
	 */
	update(opts?: UpdateOpts): Promise<void>;

	/**
	 * Invalidate the model at the given keypath. This will cause blcktive to check for any changes that may have happened directly to the data without going through a set or array method.
	 * @param keypath
	 * @param opts
	 */
	update(keypath: string, opts?: UpdateOpts): Promise<void>;
	...
	use(...plugins: Plugin[]): blcktive;

	/** The registries that are inherited by all instance. */
	static defaults: Registries<blcktive>;
	...
	/** Create a new component with this constructor as a starting point. */
	static extend(): Static<blcktive>;
	static extend<T extends ExtendOpts<any> & ValueMap, U extends Readonly<Array<ExtendOpts<any> & ValueMap>>>(opts: T, ...more: U): Static<blcktive & Merge<T, U, ExtendOpts>>;

	/** Create a new component with this constructor as a starting point using the given constructor. */
	static extendWith<U extends blcktive<U>, V extends InitOpts<U> = InitOpts<U>, W extends ExtendOpts<U> = ExtendOpts<U>>(c: Constructor<U, V>, opts?: W): Static<blcktive<U> & U>;
	...
	/** The blcktive constructor used to create this constructor. */
	static blcktive: typeof blcktive;
	/** The parent constructor used to create this constructor. */
...
export module blcktive {
	...
	/** When true, causes blcktive to emit warnings. Defaults to true. */
	let DEBUG: boolean;
	...
	/** true if blcktive detects that this environment supports svg. */
	const svg: boolean;

	/** The build version of blcktive. */
	const VERSION: string;

	/** Setting this to false will prevent blcktive from printing a welcome console message when the first instance is created. */
	let WELCOME_MESSAGE: false|undefined;

	/**
	 * The current operation promise is available to things like observers and decorators using blcktive.tick,
	 * which will return undefined if there is not currently an operation in progress.
	 */
	const tick: undefined | Promise<void>;

	/**
	 * Add blcktive-managed CSS to the managed style tag. This effectively global CSS managed by the blcktive constructor,
